    ...
    subcommand("uninstall")
        ...
        .arg(
            Arg::new("spec")
                .value_name("SPEC")
                .num_args(0..)
                .add::<clap_complete::dynamic::ArgValueCompleter>(
                clap_complete::dynamic::ArgValueCompleter::new(|| get_installed_crates()),
            ),
        )
...
fn get_installed_crates() -> Vec<clap_complete::dynamic::CompletionCandidate> {
    get_installed_crates_().unwrap_or_default()
}

fn get_installed_crates_() -> Option<Vec<clap_complete::dynamic::CompletionCandidate>> {
    ...
    for (_, v) in tracker.all_installed_bins() {
        for bin in v {
            candidates.push(clap_complete::dynamic::CompletionCandidate::new(bin));
        }
    }
